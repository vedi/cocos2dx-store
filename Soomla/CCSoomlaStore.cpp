...
namespace soomla {
    ...
    void CCSoomlaStore::initialize(CCStoreAssets *storeAssets, cocos2d::CCDictionary *storeParams) {
        ...
        if (initialized) {
            CCStoreEventDispatcher::getInstance()->onUnexpectedStoreError(CCInteger::create(0), true);
    ...
    void CCSoomlaStore::buyMarketItem(const char *productId, const char *payload, CCError **error) {
        ...
        CCStoreEventDispatcher::getInstance()->onMarketPurchase(item, CCString::create(payload), CCDictionary::create());

        item->give(1);

        //complete purchasing routine
        CCStoreEventDispatcher::getInstance()->onItemPurchased(item, CCString::create(payload));
    }
}