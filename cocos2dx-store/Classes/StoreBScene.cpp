...
void StoreBScene::menuChooseCallback(CCObject* pSender)
{
    if (pSender)
    {
        ...
        if (tag == 0) {
            ...
            CCStoreController::sharedStoreController()->buyMarketItem(productId, &soomlaError);
            if (soomlaError) {
                CCStoreUtils::logException("StoreBScene::menuChooseCallback", soomlaError);
                return;
            }
        }
    }
...
void StoreBScene::setCurrencyBalanceLabel() {
    ...
    if (soomlaError) {
        CCStoreUtils::logException("StoreBScene::setCurrencyBalanceLabel()", soomlaError);
        balance = 0;
    }
...
void StoreBScene::createListViewItem(CCPoint& origin, CCMenu* menu, CCSize& visibleSize, int tag, const char* img) {
    ...
    if (itemId == "ERROR") {
        ...
        if (soomlaError) {
            CCStoreUtils::logException("StoreBScene::createListViewItem", soomlaError);
            return;
        }
